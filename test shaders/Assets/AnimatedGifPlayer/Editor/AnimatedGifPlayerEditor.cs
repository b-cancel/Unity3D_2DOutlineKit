...
[InitializeOnLoad]
[CustomEditor(typeof(AnimatedGifPlayer))]
public class AnimatedGifPlayerEditor : Editor {
    private Rect _rt;
    private AnimatedGifPlayer _component;
    ...
    public override void OnInspectorGUI() {
        var inspectorWidth = EditorGUIUtility.currentViewWidth - 50; // EditorGUIUtility.currentViewWidth doesn't take scroll bar into account.
        //target graphic
        ...
        if (!(_component.TargetComponent is MeshRenderer) && !(_component.TargetComponent is RawImage) &&
            !(_component.TargetComponent is SpriteRenderer)) {
        ...
        if (_advancedOptionsFoldout) {
            ...
            //playback speed
            content = new GUIContent("Override Time.timeScale", "If disabled playback speed scales with Time.timeScale.");
            ...
            content = new GUIContent("Playback Speed", "Playback speed of the Gif. (1 = normal speed as set by the Gif)");
